...
def _run_all_checks(
    ...
    findings.extend(checks.check_aliases(rows, columns_present))
    findings.extend(checks.check_examples(rows, columns_present))
    findings.extend(checks.check_enumeration_consistency(rows, columns_present))
    findings.extend(checks.check_precondition_values(rows, columns_present))
    findings.extend(checks.check_unit_hygiene(rows, columns_present))
    findings.extend(checks.check_boolean_enumeration(rows, columns_present))
    findings.extend(checks.check_label_quality(rows, columns_present))
    findings.extend(checks.check_description_present(rows, columns_present))
    findings.extend(checks.check_section_runs(rows, columns_present))
